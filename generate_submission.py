...
'''.strip()
    ...
    # Use local Kaggle dataset path for offline submission
    hf_model_name = config['model_name']
    local_model_dir = hf_model_name.split("/")[-1]  # e.g. "t5-small", "opus-mt-mul-en"
    kaggle_model_path = f"/kaggle/input/kaggle-models/{local_model_dir}"
    config_code = f'''
# === Configuration (auto-generated from best experiment: {exp_name}) ===
# HuggingFace model: {hf_model_name}
# Using local path for offline Kaggle submission (model pre-downloaded in kaggle_models/)
...
try:
    ...
    print("Could not set src_lang/tgt_lang, using default")
'''
    model_load_code += '''
...
def generate_inference_only_notebook(exp_name: str, config: dict, model_dir: str, output_path: str):
    """推論専用のipynbを生成 (学習コード無し、fine-tuned重みのみ使用)"""
    nb = nbformat.v4.new_notebook()
    nb.metadata["kernelspec"] = {
        "display_name": "Python 3",
        "language": "python",
    ...
    # --- Cell 0: Title ---
    cells.append(nbformat.v4.new_markdown_cell(
        f"# Akkadian-to-English Translation (Inference Only)\n"
        f"## Model: `{config['model_name']}` ({exp_name})\n"
        f"Fine-tuned weights loaded from Kaggle Dataset. No training required.\n\n"
        f"Auto-generated by `generate_submission.py --inference-only`"
    ))

    # --- Cell 1: Install dependencies ---
    install_packages = ["transformers", "sentencepiece", "protobuf"]
    if config.get("use_lora"):
        install_packages.extend(["peft"])
    cells.append(nbformat.v4.new_code_cell(
        f"!pip install -q {' '.join(install_packages)}"
    ))
...
'''.strip()
    ...
    # --- Cell 3: Configuration ---
    # model_dir basename for Kaggle dataset path
    model_basename = os.path.basename(model_dir.rstrip("/"))
    kaggle_model_path = f"/kaggle/input/kaggle-models/{model_basename}"
    config_code = f'''
# === Configuration (inference only) ===
# Fine-tuned model weights (uploaded as Kaggle Dataset)
...
'''
    ...
    # --- Cell 6: Load test data only ---
    load_data_code = '''
# Load test data only (no training needed)
test_df = pd.read_csv(os.path.join(DATA_DIR, "test.csv"))
...
if TASK_PREFIX:
    test_src = [TASK_PREFIX + s for s in test_src]
print(f"Sample input: {test_src[0][:100]}")
'''
    cells.append(nbformat.v4.new_code_cell(load_data_code))
...
'''
    ...
    # --- Cell 8: Simple Dataset for inference ---
    dataset_code = '''
class InferenceDataset(Dataset):
    def __init__(self, sources, tokenizer, max_length):
        self.sources = sources
        self.tokenizer = tokenizer
...
test_dataset = InferenceDataset(test_src, tokenizer, MAX_SOURCE_LENGTH)
test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False)
...
'''
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        nbformat.write(nb, f)
    print(f"[INFO] Inference-only notebook saved to {output_path}")
...
def main():
    ...
    parser.add_argument("--inference-only", action="store_true",
                        help="Generate inference-only notebook (no training loop)")
    parser.add_argument("--model-dir", type=str, default="results/final_model",
                        help="Path to fine-tuned model directory (for --inference-only)")
    parser.add_argument("--model-name", type=str, default=None,
                        help="Model config name (for --inference-only, auto-detected from log if omitted)")
    ...
    if args.inference_only:
        ...
        if args.model_name:
            ...
            print("[ERROR] Specify --model-name or provide --log for auto-detection")
            return
        generate_inference_only_notebook(exp_name, config, args.model_dir, args.output)
    else:
        # 従来モード (学習+推論ノートブック)
